...
class GtkUI(object):
    def __init__(self, args):
        ...
        # Make sure gtk3.conf has at least the defaults set
        self.config = ConfigManager('gtk3.conf', DEFAULT_PREFS)

        # Make sure the gtkui state folder has been created
        if not os.path.exists(os.path.join(get_config_dir(), 'gtk3_state')):
            os.makedirs(os.path.join(get_config_dir(), 'gtk3_state'))
