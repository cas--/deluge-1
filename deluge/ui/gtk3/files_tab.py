...
from .common import listview_replace_treestore, load_pickled_state_file, reparent_iter, save_pickled_state_file
        ...
        for key, value in split_files.items():
            if key.endswith('/'):
                ...
                self.treestore.append(
                    parent_iter,
                    [key, value[1]['size'], '', 0, 0, value[0], mime_icon],
                )
                ...
                for i, p in enumerate(parent_path):
                    ...
                    if not p_itr:
                        ...
                        p_itr = self.treestore.append(
                            p_itr, [parent_path[i] + '/', 0, '', 0, 0, -1, G_ICON_DIRECTORY],
                        )
            ...
            if new_folder_iter:
                ...
                if new_split:
                    for ns in new_split[:-1]:
                        parent = self.treestore.append(parent, [ns + '/', 0, '', 0, 0, -1, G_ICON_DIRECTORY])
