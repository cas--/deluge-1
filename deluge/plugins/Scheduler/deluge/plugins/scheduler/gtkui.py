...
class SchedulerSelectWidget(Gtk.DrawingArea):
        ...
        for y in range(7):
            for x in range(24):
                context.set_source_rgba(
                    ...
                    self.colors[self.button_state[x][y]][2], 0.5,
                )
                context.rectangle(
                    width * (6 * x / 145 + 1 / 145), height * (6 * y / 43 + 1 / 43),
                    6 * width / 145, 5 * height / 43,
                )
    ...
    def get_point(self, event):
        ...
        x = int((event.x - size[0].width * 0.5 / 145) / (6 * size[0].width / 145))
        y = int((event.y - size[0].height * 0.5 / 43) / (6 * size[0].height / 43))
...
class GtkUI(Gtk3PluginBase):
    ...
    def on_apply_prefs(self):
        ...
        self.on_scheduler_event(self.state)
    ...
    def create_prefs_page(self):
        ...
        vbox_days = Gtk.VBox()
        for day in DAYS:
            vbox_days.pack_start(Gtk.Label(day, xalign=0), False, False, 0)
        ...
        label = Gtk.Label(_('Maximum Download Speed (KiB/s):'))
        ...
        label = Gtk.Label(_('Maximum Upload Speed (KiB/s):'))
        label.set_alignment(0.0, 0.6)
...
#        eventbox.modify_bg(Gtk.StateFlags.NORMAL, Gdk.color_parse('#EDD400'))
        eventbox.add(table)
        frame = Gtk.Frame()
        label = Gtk.Label()
        ...
        frame.set_label_widget(label)
        frame.set_margin_left(15)
#        frame.modify_bg(Gtk.StateFlags.NORMAL, Gdk.color_parse('#CDB400'))
